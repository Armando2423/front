...
function Users() {
    ...
    try {
      ...
      const response = await fetch("https://back-3lko.onrender.com/auth/suscripcionMod", {
        ...
        body: JSON.stringify({
          ...
          nombre: user.nombre,
          mensaje: `Gracias ${user.nombre} por usar mi PWA. Mensaje: ${message}`,
        }),
      });
  ...
  return (
    <div className="page-container">
      ...
      {userRole === "admin" ? (
        <div>
          <h2 style={{textAlign: 'center', color: 'white'}}>📋 Usuarios en Mongo Atlas...</h2>
          {isLoading ? (
        ...
        <p style={{ color: 'white', fontSize: '20px' }}>Gracias {userNombre} por usar mi PWA!! 😊</p>
      )}
    </div>
  );
}
